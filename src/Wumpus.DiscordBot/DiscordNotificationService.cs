...
using Wumpus.NotificationClient;
using Wumpus.Shared.DTOs;
using Wumpus.Shared.Models;

namespace Wumpus.DiscordBot;

/// <summary>
/// Background service that polls Wumpus notifications and sends error reports to Discord.
/// </summary>
public class DiscordNotificationService : BackgroundService
{
    ...
    private WumpusNotificationClient? _wumpusClient;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            await InitializeWumpusClientAsync();
    ...
    private async Task InitializeWumpusClientAsync()
    {
        var webApiUrl = _configuration["Wumpus:WebApiUrl"] ?? "http://localhost:1975";
        var subscriberName = _configuration["Wumpus:SubscriberName"] ?? "Discord Bot";

        _logger.LogInformation(
            "Initializing Wumpus notification client with subscriber ID {SubscriberId}...",
            _config!.SubscriberId);

        _wumpusClient = new WumpusNotificationClient(webApiUrl);

        // Try to load existing subscriber, or register if not found
        try
        {
            await _wumpusClient.LoadSubscriberAsync(_config.SubscriberId);
            _logger.LogInformation("Loaded existing Wumpus subscriber: {Name}", subscriberName);
        }
        catch
        {
            _logger.LogInformation("Subscriber not found, registering new subscriber...");
            await _wumpusClient.RegisterAsync(subscriberName, heartbeatTimeoutMinutes: 15);
            _logger.LogInformation("Registered new Wumpus subscriber with ID {SubscriberId}", _wumpusClient.SubscriberId);

            if (_wumpusClient.SubscriberId != _config.SubscriberId)
            {
                _logger.LogWarning(
                    "Registered subscriber ID {ActualId} doesn't match config {ConfigId}. " +
                    "Update config/discord-bot.json with subscriberId: \"{ActualId}\"",
                    _wumpusClient.SubscriberId,
                    _config.SubscriberId,
                    _wumpusClient.SubscriberId);
            }
        }
