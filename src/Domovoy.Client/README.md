...
A simple C# client library for sending crash reports to a Domovoy crash reporting server.

## Installation

Add a reference to the `Domovoy.Client` project in your application:

```bash
dotnet add reference path/to/Domovoy.Client/Domovoy.Client.csproj
```
...
var options = new DomovoyClientOptions
{
...
using var client = new DomovoyClient(options);
...
var options = new DomovoyClientOptions
{
    // Required: The base URL of your Domovoy server
    ServerUrl = "https://crashes.example.com",
...
- `DomovoyClient` is thread-safe for sending crash reports
